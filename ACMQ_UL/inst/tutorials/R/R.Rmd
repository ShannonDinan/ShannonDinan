...
## R
Ce tutoriel est conçu pour vous aider à découvrir `R`. Suffit de lire le texte et cliquer sur "Next Topic" pour aller à la prochaine section. Les sections "Exercices" sont interactives. De cette manière, lorsque vous cliquez sur "Run code" vous aurez un retour similaire à ce que vous allez voir dans la console. Il n'y a qu’une seule exception à la section "Manuel d'instruction". Une fois que vous exécuté, vous verrez la réponse à la ligne de code `?ifelse` dans la fenêtre "Help" (en bas à droite) de votre `RStudio` ou `RStudio Cloud`. 
...
Un objet logique (ou "booléen") prend seulement deux valeurs: `TRUE` / `FALSE`. L'objet logique sera particulièrement utile pour comparer des données numériques.
