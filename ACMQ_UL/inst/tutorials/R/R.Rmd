---
title: "ACMQ: Tutoriel R pour POL 2000"
...
Ce tutoriel est conçu pour vous aider à découvrir `R`. Suffit de lire le texte et cliquer sur "Next Topic" pour aller à la prochaine section. Certains éléments de ce tutoriel sont interactifs. De cette manière, si vous écrivez des lignes de code dans ce tutoriel, la réponse se trouvera dans votre `RStudio` ou `RStudio Cloud`. Le cas échéant, nous vous invitons à consulter les fenêtres de votre `RStudio` ou `RStudio Cloud`. La première exemple de ceci sera à la section "Manuel d'instruction". Une fois exécuté, vous verrez la réponse à la ligne de code `?ifelse` dans la fenêtre "Help" (en bas à droite) de votre `RStudio` ou `RStudio Cloud`. D'autre réponses peuvent apparaître dans la console.

Maintenant que vous comprenez le fonctionnement de ce tutoriel, qu'est-ce `R`? 
...
**ATTENTION!** Si vous avez réussi à faire rouler ce tutoriel, vous avez fort probablement déjà installé `R` et `RStudio` sur votre ordinateur personnel ou vous vous êtes créé un compte sur `RStudio Cloud`si vous suivez la version à distance de ce cours. Cette section est donc simplement à titre informatif. De cette manière, pas besoin de cliquer sur les hyperliens. Suffit de lire l'information et passer à la prochaine section.

Typiquement, pour analyser des données avec `R`, il est nécessaire d’installer trois logiciels complémentaires. Or, aux fins de la version en ligne de ce cours, il est uniquement nécessaire de créer un compte `RStudio Cloud`. Ceci est un programme infonuagique qui permet de faire l’analyse de données sans avoir à télécharger des programmes sur votre ordinateur personnel. Tous les travaux et exercices avec R dans ce cours se feront à partir de `RStudio Cloud`.

Si vous ne l'avez pas déjà fait, pour créer votre compte, cliquez [ici](https://login.rstudio.cloud/register?redirect=https%3A%2F%2Fclient.login.rstudio.cloud%2Foauth%2Flogin%3Fshow_auth%3D0%26show_login%3D0%26show_setup%3D0).
...
Le Graphique montre une capture d'écran de `RStudio`. L'analyste peut analyser ses données en mode interactif dans la "Console", en cliquant sur la fenêtre en bas à gauche, et en y tapant ses commandes suivies par la touche "Entrée."
...
Chaque commande du langage `R` est accompagnée d'un manuel d'instruction. Pour consulter ce document de référence, il faut écrire le nom de la commande précédée d'un point d'interrogation. Par exemple, taper la commande suivante dans la console fait apparaître le manuel copié dans le Graphique :
...
Pour télécharger et installer ces librairies, il faut utiliser la fonction `install.packages`. Dans le contexte de ce livre, nous utiliserons cinq librairies, en plus des commandes de base. Dans le contexte du cours POL 2000, nous ajoutons deux autres libraries.

Vous pouvez installer les librairies nécessaires pour compléter le livre ACMQ et le cours POL 2000 en copiant le code suivant dans la fenêtre "Console" de `RStudio`. **ATTENTION!** Il ne sera pas possible de télécharger ces librairies lorsque ce tutoriel fonctionne. Ceci s'explique du fait que `RStudio` fonctionne actuellement (par exemple, si vous retournez voir votre `RStudio`, vous verrez un bouton rouge "Stop" dans la console. Ceci nous indique que `RStudio` exécute actuellement une ligne de code. De cette manière, si vous tentez de  télécharger ces librairies, `RStudio` va uniquement les mettre dans la file d'attente de commandes à exécuter. Je vous suggère donc plutôt de copier ces lignes de code dans un Script.R pour les télécharger plus tard. Enfin, sachez que cette opération peut prendre quelques minutes :
...
**ATTENTION!** Si vous suivez la version à distance de ce cours, les exercices et devoirs que vous aurez à faire **contiendront déjà les librairies nécessaires à leur réalisation**. Vous n'aurez donc **pas**, en temps normal, à ajouter des libraires au code. 
...
```{r, eval = FALSE, echo=TRUE}
url <- 'https://github.com/ShannonDinan/ShannonDinan/blob/main/ACMQ_UL/inst/tutorials/R/data/titanic.csv'
titanic <- read.csv(url)
head(titanic)
```

**ATTENTION!** Si vous suivez la version à distance de ce cours, les exercices et devoirs que vous aurez à faire **contiendront déjà les données nécessaires à leur réalisation**. Vous n'aurez donc **pas**, en temps normal, à changer d'arborescence. 
